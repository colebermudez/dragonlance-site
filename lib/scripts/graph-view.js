...
class GraphAssembly {
    ...
    static minRadius = 0;
    static init(e) {
        (GraphAssembly.nodeCount = e.nodeCount), (GraphAssembly.linkCount = e.linkCount);
        ...
        (GraphAssembly.radii = new Float32Array(e.radii)),
            ...
            Module._Init(
                ...
                batchFraction,
                dt,
                e.graphOptions.attractionForce,
                e.graphOptions.linkLength,
                e.graphOptions.repulsionForce,
                e.graphOptions.centralForce
            );
    }
    ...
    static loadState() {
        ...
        if ((e && (r = new Float32Array(Object.values(JSON.parse(e)))), !r || !e || r.length != 2 * GraphAssembly.nodeCount)) {
            ...
            for (let t = 0; t < GraphAssembly.nodeCount; t++) {
                ...
                (r[2 * t] = Math.cos((t / GraphAssembly.nodeCount) * 7.41 * 2 * Math.PI) * a), (r[2 * t + 1] = Math.sin((t / GraphAssembly.nodeCount) * 7.41 * 2 * Math.PI) * a);
            }
        }
        let t = 1 / 0,
            a = -1 / 0,
        ...
        for (let e = 0; e < GraphAssembly.nodeCount - 1; e += 2) {
            let o = { x: r[e], y: r[e + 1] };
            (t = Math.min(t, o.x)), (a = Math.max(a, o.x)), (s = Math.min(s, o.y)), (i = Math.max(i, o.y));
        }
    ...
    static free() {
        Module._free(GraphAssembly.#e), Module._free(GraphAssembly.#t), Module._free(GraphAssembly.#a), Module._free(GraphAssembly.#s), Module._FreeMemory();
    }
...
class GraphRenderWorker {
    ...
    #c;
    constructor() {
        (this.canvas = document.querySelector("#graph-canvas")), (this.canvasSidebar = void 0);
        try {
    ...
    fitToRect(e) {
        let r = e.minX,
            ...
            a = e.maxX - r,
            s = e.maxY - t,
            i = 1 / Math.min(a / this.width, s / this.height);
        (this.cameraScale = i), (this.cameraOffset = { x: this.width / 2 - (e.minX + a / 2) * i, y: this.height / 2 - (e.minY + s / 2) * i });
    }
    fitToNodes() {
        this.fitToRect(startingCameraRect);
    }
    sampleColor(e) {
        ...
        document.body.appendChild(r), r.style.setProperty("display", "none"), r.style.setProperty("color", "var(" + e + ")");
    ...
    resizeCanvas(e, r) {
        this.worker.postMessage({ type: "resize", width: e, height: r }), (this.#l = e), (this.#c = r);
    }
    autoResizeCanvas() {
    ...
    set cameraOffset(e) {
        (this.#i = e), this.#g(e, this.cameraScale);
    }
    set cameraScale(e) {
        (this.#o = e), this.#g(this.cameraOffset, e);
    }
    get cameraOffset() {
        return this.#i;
    }
    ...
    set hoveredNode(e) {
        (this.#n = e), this.#m(e, this.#h);
    }
    set grabbedNode(e) {
        (this.#h = e), this.#m(this.#n, e);
    }
    set activeNode(e) {
    ...
    set width(e) {
        (this.#l = e), this.resizeCanvas(e, this.#c);
    }
    set height(e) {
        (this.#c = e), this.resizeCanvas(this.#l, e);
    }
    get height() {
        return this.#c;
    }
...
function updateGraph() {
    if (
        running &&
        !graphRenderer.canvasSidebar.classList.contains("is-collapsed") &&
        ...
        averageFPS < 0.8 * targetFPS &&
            batchFraction > minBatchFraction &&
            ((batchFraction = Math.max(batchFraction - 0.5 / targetFPS, minBatchFraction)), (GraphAssembly.batchFraction = batchFraction), (GraphAssembly.repulsionForce = graphData.graphOptions.repulsionForce / batchFraction)),
        averageFPS > 1.2 * targetFPS &&
            batchFraction < 1 &&
            ((batchFraction = Math.min(batchFraction + 0.5 / targetFPS, 1)), (GraphAssembly.batchFraction = batchFraction), (GraphAssembly.repulsionForce = graphData.graphOptions.repulsionForce / batchFraction)),
        0 != scrollVelocity)
    ) {
        graphRenderer.getCameraCenterWorldspace();
        Math.abs(scrollVelocity) < 0.001 && (scrollVelocity = 0), zoomGraphViewAroundPoint(mouseWorldPos, scrollVelocity), (scrollVelocity *= 0.65);
    }
}
function zoomGraphViewAroundPoint(e, r, t = 0.15, a = 15) {
    let s = graphRenderer.getCameraCenterWorldspace();
    if (
        ((graphRenderer.cameraScale = Math.max(Math.min(graphRenderer.cameraScale + r * graphRenderer.cameraScale, a), t)),
        graphRenderer.cameraScale != t && graphRenderer.cameraScale != a && scrollVelocity > 0 && null != mouseWorldPos.x && null != mouseWorldPos.y)
    ) {
...
function initializeGraphEvents() {
    ...
    let o = { x: 0, y: 0 },
        ...
        m = !1,
        g = !1,
        u = document.querySelector(".graph-view-container"),
        y = -1;
    u.addEventListener("pointerenter", function (r) {
        ...
        function v(e) {
            ...
            if (2 == e.touches?.length) {
                ...
                let l = (o - t) / t;
                scaleGraphViewAroundPoint(graphRenderer.vecToWorldspace(n), 1 + l, 0.15, 15), (graphRenderer.cameraOffset = { x: graphRenderer.cameraOffset.x + d.x, y: graphRenderer.cameraOffset.y + d.y }), (t = o);
            }
        }
        function f(r) {
        ...
        u.addEventListener("wheel", function (e) {
            let r = 0.09;
            e.deltaY > 0 ? (scrollVelocity >= -0.09 && (scrollVelocity = -0.09), (scrollVelocity *= 1.4)) : (scrollVelocity <= r && (scrollVelocity = r), (scrollVelocity *= 1.4));
        }),
...
window.addEventListener("load", () => {
    waitLoadScripts(["pixi", "graph-data", "graph-render-worker", "graph-wasm"], () => {
        (Module.onRuntimeInitialized = initializeGraphView), setTimeout(() => Module.onRuntimeInitialized(), 300);
    });
});
